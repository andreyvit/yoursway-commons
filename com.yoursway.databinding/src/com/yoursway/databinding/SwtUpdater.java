 ...
 * @since 1.1
 */
public abstract class SwtUpdater {
	
	private class PrivateInterface implements DisposeListener, Runnable, IChangeListener {
		
		// DisposeListener implementation
		public void widgetDisposed(DisposeEvent e) {
			SwtUpdater.this.dispose();
		}
	 ...
	 * 
	 * @param disposable control to update
	 */
	public SwtUpdater(Widget disposable) {
		this.disposable = disposable;
		disposable.addDisposeListener(privateInterface);
		makeDirty();
	}
	 ...
	 */
	public void dispose() {
		disposable.removeDisposeListener(privateInterface);
	...
	protected final void makeDirty() {
		if (!dirty) {
			dirty = true;
			stopListening();
			SWTUtil.runOnce(disposable.getDisplay(), privateInterface);
		}
	}
