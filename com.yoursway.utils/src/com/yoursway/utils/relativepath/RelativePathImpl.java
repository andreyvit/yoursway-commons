...
public class RelativePathImpl implements RelativePath, Cloneable, Serializable {
	...
	public static final RelativePathImpl EMPTY = new RelativePathImpl(EMPTY_STRING);
	...
	public static final RelativePathImpl ROOT = new RelativePathImpl(ROOT_STRING);
	 ...
	 * @see RelativePath#toPortableString()
	 * @since 3.1
	 */
	public static RelativePath fromOSString(String pathString) {
		return new RelativePathImpl(pathString);
	}
	 ...
	 * @since 3.1
	 */
	public static RelativePath fromPortableString(String pathString) {
		int firstMatch = pathString.indexOf(DEVICE_SEPARATOR) +1;
		//no extra work required if no device characters
		if (firstMatch <= 0)
			return new RelativePathImpl().initialize(null, pathString);
		...
		if (pathString.indexOf(DEVICE_SEPARATOR) == -1) 
			return new RelativePathImpl().initialize(devicePart, pathString);
		...
		return new RelativePathImpl().initialize(devicePart, new String(chars, 0, writeOffset));
	}
	
	/* (Intentionally not included in javadoc)
	 * Private constructor.
	 */
	private RelativePathImpl() {
		// not allowed
	}
	 ...
	 */
	RelativePathImpl(String fullPath) {
		String devicePart = null;
		if (WINDOWS) {
	 ...
	 */
	RelativePathImpl(String device, String path) {
		if (WINDOWS) {
			//convert backslash to forward slash
	 ...
	 * Private constructor.
	 */
	private RelativePathImpl(String device, String[] segments, int _separators) {
	 ...
	 */
	public RelativePath addFileExtension(String extension) {
		...
		return new RelativePathImpl(device, newSegments, separators);
	}

	/* (Intentionally not included in javadoc)
	 * @see IPath#addTrailingSeparator
	 */
	public RelativePath addTrailingSeparator() {
		if (hasTrailingSeparator() || isRoot()) {
			return this;
		}
		//XXX workaround, see 1GIGQ9V
		if (isEmpty()) {
			return new RelativePathImpl(device, segments, HAS_LEADING);
		}
		return new RelativePathImpl(device, segments, separators | HAS_TRAILING);
	}

	/* (Intentionally not included in javadoc)
	 * @see IPath#append(IPath)
	 */
	public RelativePath append(RelativePath tail) {
		...
		RelativePathImpl result = new RelativePathImpl(device, newSegments, (separators & (HAS_LEADING | IS_UNC)) | (tail.hasTrailingSeparator() ? HAS_TRAILING : 0));
	 ...
	 */
	public RelativePath append(String tail) {
		...
		if (tail.indexOf(SEPARATOR) == -1 && tail.indexOf("\\") == -1 && tail.indexOf(DEVICE_SEPARATOR) == -1) { //$NON-NLS-1$
			...
			return new RelativePathImpl(device, newSegments, separators & ~HAS_TRAILING);
		}
		//go with easy implementation
		return append(new RelativePathImpl(tail));
	}
	...
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (!(obj instanceof RelativePathImpl))
			return false;
		RelativePathImpl target = (RelativePathImpl) obj;
	 ...
	 */
	private RelativePath initialize(String deviceString, String path) {
		if (path == null)
	 ...
	 */
	public boolean isPrefixOf(RelativePath anotherPath) {
		if (device == null) {
	...
	public boolean isValidPath(String path) {
		RelativePathImpl test = new RelativePathImpl(path);
	 ...
	 */
	public RelativePath makeAbsolute() {
		if (isAbsolute()) {
			return this;
		}
		RelativePathImpl result = new RelativePathImpl(device, segments, separators | HAS_LEADING);
	 ...
	 */
	public RelativePath makeUNC(boolean toUNC) {
		...
		return new RelativePathImpl(toUNC ? null : device, segments, newSeparators);
	}

	/* (Intentionally not included in javadoc)
	 * @see IPath#matchingFirstSegments(IPath)
	 */
	public int matchingFirstSegments(RelativePath anotherPath) {
        if (anotherPath == null)
            throw new NullPointerException("anotherPath");
	 ...
	 */
	public RelativePath removeFileExtension() {
	 ...
	 */
	public RelativePath removeFirstSegments(int count) {
		if (count == 0)
			return this;
		if (count >= segments.length) {
			return new RelativePathImpl(device, NO_SEGMENTS, 0);
		}
		if (count <= 0)
	 ...
	 */
	public RelativePath removeLastSegments(int count) {
		if (count == 0)
			return this;
		if (count >= segments.length) {
			//result will have no trailing separator
			return new RelativePathImpl(device, NO_SEGMENTS, separators & (HAS_LEADING | IS_UNC));
		}
		if(count <= 0)
		    throw new IllegalArgumentException();
		int newSize = segments.length - count;
	 ...
	 */
	public RelativePath removeTrailingSeparator() {
		if (!hasTrailingSeparator()) {
			return this;
		}
		return new RelativePathImpl(device, segments, separators & (HAS_LEADING | IS_UNC));
	}
	 ...
	 */
	public RelativePath setDevice(String value) {
		if (value != null) {
			if(value.indexOf(RelativePath.DEVICE_SEPARATOR) != (value.length() - 1))
			    throw new AssertionError("Last character should be the device separator"); //$NON-NLS-1$
		}
		//return the receiver if the device is the same
		if (value == device || (value != null && value.equals(device)))
			return this;

		return new RelativePathImpl(value, segments, separators);
	}
	 ...
	 */
	public RelativePath uptoSegment(int count) {
		if (count == 0)
			return new RelativePathImpl(device, NO_SEGMENTS, separators & (HAS_LEADING | IS_UNC));
		if (count >= segments.length)
			return this;
		if(count <= 0)
		    throw new IllegalArgumentException("Invalid parameter to Path.uptoSegment"); //$NON-NLS-1$
